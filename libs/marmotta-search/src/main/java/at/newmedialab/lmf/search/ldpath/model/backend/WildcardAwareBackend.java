...
public class WildcardAwareBackend extends ContextAwareSesameConnectionBackend {
		...
		try {
			if (property instanceof Literal) {
				if (property.stringValue().endsWith("*") || property.stringValue().endsWith("%")) {
					return listObjectsWildcard(connection, (Resource) subject, property.stringValue(), includeInferred,
							contexts);
				}
			}
			...
			throw new IllegalArgumentException(
					String.format("Subject needs to be a URI or blank node, property a URI node "
							+ "(types: [subject: %s, property: %s])", debugType(subject), debugType(property)),
					e);
		}

	}

	protected Collection<Value> listObjectsWildcard(RepositoryConnection connection, Resource subject, String property,
		...
		RepositoryResult<Statement> qResult = connection.getStatements(merge(subject, valueFactory), null, null,
		...
		try {
			...
			while (qResult.hasNext()) {
				...
				if (stmt.getPredicate().stringValue().startsWith(prop)) {
					...
					// String propValue = String.format("%s"+SEPARATOR+"%s",
	...
	protected Collection<Value> listSubjectsInternal(final RepositoryConnection connection,
			org.openrdf.model.URI property, Value object, boolean includeInferred, Resource... contexts)
			throws RepositoryException {
		final ValueFactory valueFactory = connection.getValueFactory();

		Set<Value> result = new HashSet<>();
		// Be aware: Property and object are not properly provided - need to swap the two values in the
		// subsequent call 
		RepositoryResult<Statement> qResult = connection.getStatements(
				null,													// we are interested in the subjects		
				merge((org.openrdf.model.URI) object, valueFactory), 		// use object from parameter list
				merge(property, valueFactory), 							// use property from parameter list
				includeInferred,			
				contexts);
		try {
			while (qResult.hasNext()) {
	...
	public Collection<Value> createDynLiteralCollection(Map<String, Collection<String>> nameValuesMap) {
		HashSet<Value> vl = new HashSet<>();
		for (String name :  nameValuesMap.keySet()) {
			for ( String value : nameValuesMap.get(name)) {
				Value val = new DynKiwiLiteral(name, value);
				vl.add(val);
			}
		}
